...
echo "[INFO] CREATE DEFAULT GRAPH AND LIVE SUBGRAPHS'"
run_virtuoso_cmd "DB.DBA.RDF_GRAPH_GROUP_CREATE ('${DOMAIN}',1);"

run_virtuoso_cmd "DB.DBA.RDF_GRAPH_GROUP_INS ('${DOMAIN}','http://${DBP_LANG}.live.dbpedia.org');"
run_virtuoso_cmd "DB.DBA.RDF_GRAPH_GROUP_INS ('${DOMAIN}','http://${DBP_LANG}.static.dbpedia.org');"
run_virtuoso_cmd "DB.DBA.RDF_GRAPH_GROUP_INS ('${DOMAIN}','http://${DBP_LANG}.dbpedia.org/ontology');"
run_virtuoso_cmd "DB.DBA.RDF_GRAPH_GROUP_INS ('${DOMAIN}','http://${DBP_LANG}.dbpedia.org/ontology/meta');"
...
do
  ...
  then
	fn=${entry##*/} # GET FILE NAME ONLY
  	run_virtuoso_cmd "ld_dir ('${STORE_DATA_DIR}', '${fn}', '${DOMAIN}');"
  fi
done
